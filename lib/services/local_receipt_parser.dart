...
class LocalReceiptParser {
  ...
  static String _extractCurrency(String text) {
    if (text.contains('¥') || text.contains('￥') ||
        text.contains('円') || text.contains('JPY')) return 'JPY';
    if (text.contains('元') || text.contains('CNY') ||
        text.contains('RMB')) return 'CNY';
  ...
  static String _guessCategory(String text) {
    ...
    final rules = {
      '餐饮': ['レストラン', '食堂', '定食', 'cafe', 'coffee',
                'restaurant', 'ラーメン', '寿司', '焼肉', '居酒屋'],
      '交通': ['バス', 'タクシー', '電車', 'suica', 'pasmo',
                'taxi', 'metro', '地铁', '公交'],
      '购物': ['スーパー', 'コンビニ', 'ドラッグ', 'mall',
                'shopping', '超市', '便利店'],
      '医疗': ['薬局', '病院', 'クリニック', 'pharmacy',
                'hospital', '药店', '医院'],
