...
class Account {
  ...
  double get balanceFloat =>
      intToAmount(balanceCache, currencyCode);
 
  String get formattedBalance =>
      formatAmount(balanceFloat, currencyCode);
...
// 把整数余额转回 float（与 Python utils/currency.py 保持一致）
double intToAmount(int value, String currency) {
  const noDecimal = {'JPY', 'KRW', 'VND'};
  if (noDecimal.contains(currency.toUpperCase())) return value.toDouble();
  return value / 100.0;
}